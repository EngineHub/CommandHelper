...
class CompilerObject {
	...
	void compile0() throws ConfigCompileException {
		...
		if (t.type == Token.TType.CONST_START) {
			...
			while ((t = consume()).type != Token.TType.RCURLY_BRACKET) {
		...
		if (t.type == Token.TType.BARE_STRING && peek().type == Token.TType.FUNC_START) {
			consume();
		...
		if (t.type == Token.TType.FUNC_END || t.type == Token.TType.COMMA) {
		...
		if (peek().type != Token.TType.FUNC_END && peek().type != Token.TType.COMMA && peek().type != Token.TType.RCURLY_BRACKET && peek().type != Token.TType.RSQUARE_BRACKET) {
		...
		if (t.type == Token.TType.BARE_STRING && peek().type == Token.TType.LABEL) {
			consume();
