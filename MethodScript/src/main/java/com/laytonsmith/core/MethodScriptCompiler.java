	...
	public static List<Token> lex(String script, File file, boolean inPureMScript) throws ConfigCompileException {
		...
		for (int i = 0; i < script.length(); i++) {
			...
			if(c == '+' && c2 == '=' && !state_in_quote){
				if(buf.length() > 0){
					token_list.add(new Token(Token.TType.UNKNOWN, buf.toString(), target));
					buf = new StringBuilder();
				}
				token_list.add(new Token(Token.TType.PLUS_ASSIGNMENT, "+=", target));
				i++;
				continue;
			}
			if(c == '-' && c2 == '=' && !state_in_quote){
				...
				token_list.add(new Token(Token.TType.MINUS_ASSIGNMENT, "-=", target));
				i++;
				continue;
			}
			if(c == '*' && c2 == '=' && !state_in_quote){
				...
				token_list.add(new Token(Token.TType.MULTIPLICATION_ASSIGNMENT, "*=", target));
				i++;
				continue;
			}
			...
			if(c == '/' && c2 == '=' && !state_in_quote){
				...
				token_list.add(new Token(Token.TType.DIVISION_ASSIGNMENT, "/=", target));
				i++;
				continue;
			}
			if(c == '.' && c2 == '=' && !state_in_quote){
				...
				token_list.add(new Token(Token.TType.CONCAT_ASSIGNMENT, "/=", target));
				i++;
				continue;
			}
			...
			if (c == '-' && c2 == '>' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.DEREFERENCE, "->", target));
				i++;
				continue;
			}
			...
			if (c == '+' && c2 == '+' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.INCREMENT, "++", target));
				i++;
				continue;
			}
			if (c == '-' && c2 == '-' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.DECREMENT, "--", target));
				i++;
				continue;
			}
			...
			if (c == '%' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.MODULO, "%", target));
				continue;
			}
			...
			if (c == '*' && c2 == '*' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.EXPONENTIAL, "**", target));
				i++;
				continue;
			}
			if (c == '*' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.MULTIPLICATION, "*", target));
				continue;
			}
			if (c == '+' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.PLUS, "+", target));
				continue;
			}
			if (c == '-' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.MINUS, "-", target));
				continue;
			}
			...
			if (c == '/' && !Character.isLetter(c2) && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.DIVISION, "/", target));
				continue;
			}
			//Logic symbols
			if (c == '>' && c2 == '=' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.GTE, ">=", target));
				i++;
				continue;
			}
			if (c == '<' && c2 == '=' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.LTE, "<=", target));
				i++;
				continue;
			}
			...
			if (c == '<' && c2 == '<' && c3 == '<' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.MULTILINE_END, "<<<", target));
				inMultiline = false;
				i++;
				i++;
				continue;
			...
			if (c == '>' && c2 == '>' && c3 == '>' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.MULTILINE_START, ">>>", target));
				inMultiline = true;
				i++;
				i++;
				continue;
			...
			if (c == '<' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.LT, "<", target));
				continue;
			}
			if (c == '>' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.GT, ">", target));
				continue;
			}
			if (c == '=' && c2 == '=' && c3 == '=' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.STRICT_EQUALS, "===", target));
				i++;
				i++;
				continue;
			}
			if (c == '!' && c2 == '=' && c3 == '=' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.STRICT_NOT_EQUALS, "!==", target));
				i++;
				i++;
				continue;
			}
			if (c == '=' && c2 == '=' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.EQUALS, "==", target));
				i++;
				continue;
			}
			if (c == '!' && c2 == '=' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.NOT_EQUALS, "!=", target));
				i++;
				continue;
			}
			if (c == '&' && c2 == '&' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.LOGICAL_AND, "&&", target));
				i++;
				continue;
			}
			if (c == '|' && c2 == '|' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.LOGICAL_OR, "||", target));
				i++;
				continue;
			}
			if (c == '!' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.LOGICAL_NOT, "!", target));
				continue;
			}
			if (c == '{' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.LCURLY_BRACKET, "{", target));
				continue;
			}
			if (c == '}' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.RCURLY_BRACKET, "}", target));
				continue;
			}
...
//            }
			...
			if (c == '.' && c2 == '.' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.SLICE, "..", target));
				i++;
				continue;
			}
			if(c == '.' && !state_in_quote){
				...
				// the compiler stage.
				token_list.add(new Token(Token.TType.DOT, ".", target));
				continue;
			}
			if (c == ':' && c2 == ':' && !state_in_quote) {
				if (buf.length() > 0) {
			...
			if (c == '[' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.LSQUARE_BRACKET, "[", target));
				in_opt_var = true;
				continue;
			}
			...
			if (c == '=' && !state_in_quote) {
				...
				if(inCommand){
					if (in_opt_var) {
						token_list.add(new Token(Token.TType.OPT_VAR_ASSIGN, "=", target));
					} else {
						token_list.add(new Token(Token.TType.ALIAS_END, "=", target));
						inCommand = false;
					}
				} else {
					token_list.add(new Token(Token.TType.ASSIGNMENT, "=", target));
				}
				continue;
			}
			if (c == ']' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.RSQUARE_BRACKET, "]", target));
				in_opt_var = false;
				continue;
			}
			if (c == ':' && !state_in_quote) {
				...
				token_list.add(new Token(Token.TType.LABEL, ":", target));
				continue;
			}
			if (c == ',' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.COMMA, ",", target));
				continue;
			}
			if (c == '(' && !state_in_quote) {
				if (buf.length() > 0) {
					token_list.add(new Token(Token.TType.FUNC_NAME, buf.toString(), target));
					buf = new StringBuilder();
				} else {
					...
					try {
						int count = 1;
						while (token_list.get(token_list.size() - count).type == Token.TType.WHITESPACE) {
							count++;
						}
						if (token_list.get(token_list.size() - count).type == Token.TType.UNKNOWN) {
							token_list.get(token_list.size() - count).type = Token.TType.FUNC_NAME;
							...
							token_list.add(new Token(Token.TType.FUNC_NAME, "__autoconcat__", target));
						}
				...
				token_list.add(new Token(Token.TType.FUNC_START, "(", target));
				continue;
			}
			if (c == ')' && !state_in_quote) {
				if (buf.length() > 0) {
				...
				token_list.add(new Token(Token.TType.FUNC_END, ")", target));
				continue;
			}
			if(c == ';' && !state_in_quote){
				if(buf.length() > 0){
				...
				token_list.add(new Token(Token.TType.SEMICOLON, ";", target));
				continue;
			}
			if (Character.isWhitespace(c) && !state_in_quote && c != '\n') {
				...
				if (token_list.size() > 0
						&& token_list.get(token_list.size() - 1).type != Token.TType.WHITESPACE) {
					token_list.add(new Token(Token.TType.WHITESPACE, " ", target));
				}
				continue;
			}
			if (c == '\'') {
				if (state_in_quote && !in_smart_quote) {
					token_list.add(new Token(Token.TType.STRING, buf.toString(), target));
					buf = new StringBuilder();
				...
				if (state_in_quote && in_smart_quote) {
					token_list.add(new Token(Token.TType.SMART_STRING, buf.toString(), target));
					buf = new StringBuilder();
					state_in_quote = false;
					in_smart_quote = false;
					continue;
				...
				if (state_in_quote) {
					...
					token_list.add(new Token(Token.TType.SEPERATOR, "\\", target));
				}
			} else if (state_in_quote) {
				buf.append(c);
				continue;
				...
				token_list.add(new Token(Token.TType.NEWLINE, "\n", target));
				in_comment = false;
				comment_is_block = false;
				continue;
			} else { //in a literal
		...
		for (int i = 0; i < token_list.size(); i++) {
			...
			Token prev2 = i - 2 >= 0 ? token_list.get(i - 2) : new Token(Token.TType.UNKNOWN, "", t.target);
			Token prev1 = i - 1 >= 0 ? token_list.get(i - 1) : new Token(Token.TType.UNKNOWN, "", t.target);
			Token next = i + 1 < token_list.size() ? token_list.get(i + 1) : new Token(Token.TType.UNKNOWN, "", t.target);

			if (t.type == Token.TType.UNKNOWN && prev1.type.isPlusMinus() && !prev2.type.isIdentifier()
					&& !prev2.type.equals(Token.TType.FUNC_END)
			...
			if (t.type.equals(Token.TType.UNKNOWN)) {
				if (t.val().charAt(0) == '/' && t.val().length() > 1) {
					...
					t.type = Token.TType.KEYWORD;
				} else {
					t.type = Token.TType.LIT;
				}
			}
			//Skip this check if we're not in pure mscript
			if(inPureMScript){
				if (t.type.isSymbol() && !t.type.isUnary() && !next.type.isUnary()) {
					if (prev1.type.equals(Token.TType.FUNC_START) || prev1.type.equals(Token.TType.COMMA)
							|| next.type.equals(Token.TType.FUNC_END) || next.type.equals(Token.TType.COMMA)
	...
	public static List<Script> preprocess(List<Token> tokenStream) throws ConfigCompileException {
		...
		for (int i = 0; i < tokenStream.size(); i++) {
			try {
				if (tokenStream.get(i).type.equals(Token.TType.NEWLINE)) {
					temp.add(new Token(Token.TType.NEWLINE, "\n", tokenStream.get(i).target));
					while (tokenStream.get(++i).type.equals(Token.TType.NEWLINE)) {
					}
				}
				if (tokenStream.get(i).type != Token.TType.WHITESPACE) {
		...
		if (temp.size() > 0 && temp.get(0).type.equals(Token.TType.NEWLINE)) {
			temp.remove(0);
		}
		...
		for (int i = 0; i < tokenStream.size(); i++) {
			Token prevToken = i - 1 >= tokenStream.size() ? tokenStream.get(i - 1) : new Token(Token.TType.UNKNOWN, "", Target.UNKNOWN);
			thisToken = tokenStream.get(i);
			Token nextToken = i + 1 < tokenStream.size() ? tokenStream.get(i + 1) : new Token(Token.TType.UNKNOWN, "", Target.UNKNOWN);
			//take out newlines between the = >>> and <<< tokens (also the tokens)
			if (thisToken.type.equals(Token.TType.ALIAS_END) && nextToken.val().equals(">>>")) {
			...
			if (thisToken.val().equals(">>>") && !prevToken.type.equals(Token.TType.ALIAS_END)) {
			...
			//it
			if (!inside_multiline || !thisToken.type.equals(Token.TType.NEWLINE)) {
				tokens1_1.add(thisToken);
			}
		}
		...
		for (int i = 0; i < tokens1_1.size(); i++) {
			if (!tokens1_1.get(i).type.equals(Token.TType.STRING) && tokens1_1.get(i).val().equals("\\") && tokens1_1.size() > i
					&& tokens1_1.get(i + 1).type.equals(Token.TType.NEWLINE)) {
			...
			if (inLeft) {
				...
				if (t.type == Token.TType.NEWLINE) {
					...
					for(int j = left.size() - 1; j >= 0; j--){
						if(left.get(j).type == Token.TType.NEWLINE){
							if(j > 0 && left.get(j - 1).type != Token.TType.WHITESPACE){
	...
	public static ParseTree compile(List<Token> stream) throws ConfigCompileException, ConfigCompileGroupException {
		...
		for (int i = 0; i < stream.size(); i++) {
			...
			Token prev1 = i - 1 >= 0 ? stream.get(i - 1) : new Token(Token.TType.UNKNOWN, "", t.target);
			Token next1 = i + 1 < stream.size() ? stream.get(i + 1) : new Token(Token.TType.UNKNOWN, "", t.target);
			Token next2 = i + 2 < stream.size() ? stream.get(i + 2) : new Token(Token.TType.UNKNOWN, "", t.target);
			Token next3 = i + 3 < stream.size() ? stream.get(i + 3) : new Token(Token.TType.UNKNOWN, "", t.target);
			...
			if (t.type.equals(Token.TType.LSQUARE_BRACKET)) {
				arrayStack.push(new AtomicInteger(tree.getChildren().size() - 1));
				continue;
			} else if (t.type.equals(Token.TType.RSQUARE_BRACKET)) {
				boolean emptyArray = false;
				if (prev1.type.equals(Token.TType.LSQUARE_BRACKET)) {
					emptyArray = true;
				}
				...
				if (minusArrayStack.size() != 0 && arrayStack.size() + 1 == minusArrayStack.peek().get()) {
					if (!next1.type.equals(Token.TType.LSQUARE_BRACKET)) { // Wait if there are more array_get's comming.
			...
			if (t.type.equals(Token.TType.FUNC_NAME)) {
				...
				tree = f;
				parents.push(f);
			} else if (t.type.equals(Token.TType.FUNC_START)) {
				if (!prev1.type.equals(Token.TType.FUNC_NAME)) {
					throw new ConfigCompileException("Unexpected parenthesis", t.target);
				}
				parens++;
			} else if (t.type.equals(Token.TType.FUNC_END)) {
				if (parens <= 0) {
					throw new ConfigCompileException("Unexpected parenthesis", t.target);
				}
				parens--;
				...
				if (minusFuncStack.size() != 0 && minusFuncStack.peek().get() == parens + 1) {
					if(next1.type.equals(Token.TType.LSQUARE_BRACKET)) {
			...
			} else if (t.type.equals(Token.TType.COMMA)) {
				if (constructCount.peek().get() > 1) {
					int stacks = constructCount.peek().get();
					int replaceAt = tree.getChildren().size() - stacks;
			...
			if (next1.type.equals(Token.TType.SLICE)) {
			...
			} else if (t.type == Token.TType.LIT) {
				...
				} else if(c instanceof CInt && next1.type == Token.TType.DOT && next2.type == Token.TType.LIT) {
				...
				constructCount.peek().incrementAndGet();
			} else if (t.type.equals(Token.TType.STRING) || t.type.equals(Token.TType.COMMAND)) {
				tree.addChild(new ParseTree(new CString(t.val(), t.target), fileOptions));
				constructCount.peek().incrementAndGet();
			} else if (t.type.equals(Token.TType.IDENTIFIER)) {
				tree.addChild(new ParseTree(new CPreIdentifier(t.val(), t.target), fileOptions));
				constructCount.peek().incrementAndGet();
			} else if(t.type.isKeyword()){
				tree.addChild(new ParseTree(new CKeyword(t.val(), t.getTarget()), fileOptions));
				constructCount.peek().incrementAndGet();
			} else if (t.type.equals(Token.TType.IVARIABLE)) {
				tree.addChild(new ParseTree(new IVariable(t.val(), t.target), fileOptions));
				constructCount.peek().incrementAndGet();
			} else if (t.type.equals(Token.TType.UNKNOWN)) {
				...
				if (t.type.equals(Token.TType.MINUS) && !prev1.type.isAtomicLit() && !prev1.type.equals(Token.TType.IVARIABLE)
						&& !prev1.type.equals(Token.TType.VARIABLE) && !prev1.type.equals(Token.TType.RCURLY_BRACKET)
						&& !prev1.type.equals(Token.TType.RSQUARE_BRACKET) && !prev1.type.equals(Token.TType.FUNC_END)
						&& (next1.type.equals(Token.TType.IVARIABLE) || next1.type.equals(Token.TType.VARIABLE) || next1.type.equals(Token.TType.FUNC_NAME))) {
					
					// Check if we are negating a value from an array, function or variable.
					if (next2.type.equals(Token.TType.LSQUARE_BRACKET)) {
						minusArrayStack.push(new AtomicInteger(arrayStack.size() + 1)); // +1 because the bracket isn't counted yet.
					} else if (next1.type.equals(Token.TType.FUNC_NAME)) {
				...
				tree.addChild(new ParseTree(c, fileOptions));
				constructCount.peek().incrementAndGet();
			} else if (t.type.equals(Token.TType.VARIABLE) || t.type.equals(Token.TType.FINAL_VAR)) {
				tree.addChild(new ParseTree(new Variable(t.val(), null, false, t.type.equals(Token.TType.FINAL_VAR), t.target), fileOptions));
				constructCount.peek().incrementAndGet();
	...
	private static void link(ParseTree tree, Set<ConfigCompileException> compilerErrors) {
		...
		if(treeFunction != null){
			...
			if(treeFunction instanceof Optimizable){
				...
				if(op.optimizationOptions().contains(Optimizable.OptimizationOption.CUSTOM_LINK)){
					try {
...
//		}
		...
		if (tree.getData().getValue().equals(oldFunctionName)
				&& (options.contains(Optimizable.OptimizationOption.OPTIMIZE_CONSTANT) || options.contains(Optimizable.OptimizationOption.CONSTANT_OFFLINE))) {
	 ...
	 * @return
	 * @throws ConfigCompileException
	 * @throws ConfigCompileGroupException This indicates
