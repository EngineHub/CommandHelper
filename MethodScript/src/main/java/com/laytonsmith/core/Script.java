...
public class Script {
    ...
    private boolean verifyLeft() throws ConfigCompileException {
        ...
        for(int i = 0; i < left.size(); i++){
            Token t = left.get(i);
            if(i == 0 && t.type == Token.TType.NEWLINE){
                continue;
            }
            if(t.type.isSymbol() && left.size() - 1 > i && left.get(i + 1).type != Token.TType.WHITESPACE){
                StringBuilder b = new StringBuilder();
                b.append(t.value);
                i++;
                Token m = left.get(i);
                ...
                t = new Token(Token.TType.STRING, b.toString(), t.target);
            ...
            if(t.type.isSymbol()){
                t = new Token(Token.TType.STRING, t.value, t.target);
            }
            if(t.type != Token.TType.WHITESPACE){
                tempLeft.add(t);
            }
        ...
        for(int i = 0; i < tempLeft.size(); i++){
            if(tempLeft.get(i).type == Token.TType.LABEL){
                hasLabel = true;
                break;
            }
        }
        if(hasLabel){
            StringBuilder b = new StringBuilder();
            int count = 0;
            while(tempLeft.get(count).type != Token.TType.LABEL){
        ...
        for (int j = 0; j < left.size(); j++) {
            ...
            Token last_token = j - 1 >= 0 ? left.get(j - 1) : new Token(Token.TType.UNKNOWN, "", t.getTarget());
            Token next_token = j + 1 < left.size() ? left.get(j + 1) : new Token(Token.TType.UNKNOWN, "", t.getTarget());
            Token after_token = j + 2 < left.size() ? left.get(j + 2) : new Token(Token.TType.UNKNOWN, "", t.getTarget());

            if (j == 0) {
                if (next_token.type == Token.TType.LABEL) {
                    this.label = t.val();
                    j--;
                    left.remove(0);
                    left.remove(0);
            ...
            if (t.type.equals(Token.TType.FINAL_VAR) && left.size() - j >= 5) {
                throw new ConfigCompileException("FINAL_VAR must be the last argument in the alias", t.target);
            }
            if (t.type.equals(Token.TType.VARIABLE) || t.type.equals(Token.TType.FINAL_VAR)) {
                Variable v = new Variable(t.val(), null, t.target);
                lastVar = t.val();
                v.setOptional(last_token.type.equals(Token.TType.LSQUARE_BRACKET));
            ...
            if (j == 0 && !t.value.startsWith("/")) {
                if (!(next_token.type == Token.TType.LABEL && after_token.type == Token.TType.COMMAND)) {
                    throw new ConfigCompileException("Expected command (/command) at start of alias."
                            + " Instead, found " + t.type + " (" + t.val() + ")", t.target);
                }
            }
            if (last_token.type.equals(Token.TType.LSQUARE_BRACKET)) {
                inside_opt_var = true;
                if (!(t.type.equals(Token.TType.FINAL_VAR) || t.type.equals(Token.TType.VARIABLE))) {
            ...
            if (!t.type.equals(Token.TType.LSQUARE_BRACKET)
                    && !t.type.equals(Token.TType.OPT_VAR_ASSIGN)
                    && !t.type.equals(Token.TType.RSQUARE_BRACKET)
                    && !t.type.equals(Token.TType.VARIABLE)
                    && !t.type.equals(Token.TType.LIT)
                    && !t.type.equals(Token.TType.COMMAND)
                    && !t.type.equals(Token.TType.FINAL_VAR)) {
                if (j - 1 > 0 && !(/*t.type.equals(TType.STRING) &&*/ left.get(j - 1).type.equals(Token.TType.OPT_VAR_ASSIGN))) {
                    throw new ConfigCompileException("Unexpected " + t.type + " (" + t.val() + ")", t.target);
                }
            }
            if (last_token.type.equals(Token.TType.COMMAND)) {
                if (!(t.type.equals(Token.TType.VARIABLE) || t.type.equals(Token.TType.LSQUARE_BRACKET) || t.type.equals(Token.TType.FINAL_VAR)
                        || t.type.equals(Token.TType.LIT) || t.type.equals(Token.TType.STRING))) {
                    throw new ConfigCompileException("Unexpected " + t.type + " (" + t.val() + ") after command", t.target);
                }
            }
            if (inside_opt_var && t.type.equals(Token.TType.OPT_VAR_ASSIGN)) {
                if (!(next_token.type.isAtomicLit() && after_token.type.equals(Token.TType.RSQUARE_BRACKET)
                        || (next_token.type.equals(Token.TType.RSQUARE_BRACKET)))) {
            ...
            if (t.type.equals(Token.TType.RSQUARE_BRACKET)) {
                if (!inside_opt_var) {
    ...
    private boolean compileLeft() {
        ...
        for (int i = 0; i < left.size(); i++) {
            ...
            } else if (t.type.equals(Token.TType.FINAL_VAR)) {
                Variable v = new Variable(t.val(), null, t.target);
                v.setFinal(true);
                cleft.add(v);
            } else if (t.type.equals(Token.TType.LSQUARE_BRACKET)) {
                if (i + 2 < left.size() && left.get(i + 2).type.equals(Token.TType.OPT_VAR_ASSIGN)) {
                    Variable v = new Variable(left.get(i + 1).val(),
                            left.get(i + 3).val(), t.target);
                    v.setOptional(true);
                    if (left.get(i + 1).type.equals(Token.TType.FINAL_VAR)) {
                        v.setFinal(true);
                    }
                    cleft.add(v);
                    i += 4;
    ...
    public void compileRight() throws ConfigCompileException, ConfigCompileGroupException {
        ...
        for (Token t : fullRight) {
            if (t.type == Token.TType.SEPERATOR) {
                right.add(temp);
