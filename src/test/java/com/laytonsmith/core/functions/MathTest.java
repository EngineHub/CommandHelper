...
public class MathTest extends AbstractIntegrationTest {
	...
	public void testRand1() {
		...
		for(int i = 0; i < 1000; i++) {
			long j = ArgumentValidation.getInt(a.exec(Target.UNKNOWN, env, null, C.onstruct(10)), t);
			if(!(j < 10 && j >= 0)) {
				fail("Expected a number between 0 and 10, but got " + j);
			}
			j = ArgumentValidation.getInt(a.exec(Target.UNKNOWN, env, null, C.onstruct(10), C.onstruct(20)), t);
			if(!(j < 20 && j >= 10)) {
