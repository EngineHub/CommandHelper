...
public class WebUtility {
    ...
    public static final class HTTPCookies{
        ...
        public String getCookies(URL url){
            ...
            for(int i = 0; i < cookies.size(); i++){
                ...
                if(cookie.httpOnly && !url.getProtocol().equals("http")) continue;
                //Or it's secure only, and we aren't in https, continue.
                if(cookie.secureOnly && !url.getProtocol().equals("https")) continue;
                
                //If we aren't in the correct domain
                if(!url.getHost().endsWith(cookie.domain)) continue;
                //Or if we aren't in the right path
                if(!url.getPath().startsWith(cookie.path)) continue;
    ...
    /**
     * Gets a web page based on the parameters specified. This is a blocking call, if you wish for it to be event driven,
     * consider using the GetPage that requires a HTTPResponseCallback.
     * @param url The url to navigate to
     * @param method The HTTP method to use
     * @param parameters The parameters to be sent. Parameters can be also specified directly in the URL, and they will be merged.
     * @param cookieStash An instance of a cookie stash to use, or null if none is needed. Cookies will automatically be added and used from this
     * instance.
    ...
    /**
     * A very simple convenience method to get a page. Only the contents
