...
public class ArgumentParser {
	...
	private class Argument {
		...
		public Argument(Character shortArg, String longArg, Type argType, String defaultVal, String description, String usageName, boolean required) {
	...
	public class ArgumentParserResults {
		...
		/**
		 * Returns true if the flag represented by this short code is
		 * set.
		 *
		 * @param flag
		 * @return
		...
		/**
		 * Returns true is the flag represented by this long code is
		 * set.
		 *
		 * @param flag
		 * @return
		...
		/**
		 * Gets the unassociated arguments passed in as a String. For
		 * instance, if the arguments were
		 * <code>These are arguments</code>, then "These are arguments"
		 * will be returned. However, assuming -c is registered as a
		 * single string type, and the arguments are
		 * <code>-c These are arguments</code>, then only "are
		 * arguments" is returned. This will return an empty string if
		...
		/**
		 * Returns the string associated with the switch represented by
		...
		/**
		 * Returns the value associated with the switch represented by
		 * this short code, pre-parsed as a double. If the switch wasn't
		...
		/**
		 * Returns the value associated with the switch represented by
		 * this long code, pre-parsed as a double. If the switch wasn't
		 * set, null is returned.
		 *
		 * @param flag
		 * @return
		...
		/**
		 * Gets the unassociated arguments passed in as a List of
		 * Strings. For instance, if the arguments were
		 * <code>These are arguments</code>, then ["These", "are",
		 * "arguments"] will be returned. However, assuming -c is
		 * registered as a single string type, and the arguments are
		 * <code>-c These are arguments</code>, then only ["are",
		 * "arguments"] is returned. This will return an empty array if
		 * no arguments were set.
		 *
		 * @return
		 */
		...
		/**
		 * Returns the list of values associated with the switch
		 * represented by this short code. If the switch wasn't set,
		 * null is returned.
		 *
		 * @param flag
		 * @return
		...
		/**
		 * Returns the list of values associated with the switch
		 * represented by this long code. If the switch wasn't set, null
		 * is returned.
		 *
		 * @param flag
		 * @return
		...
		/**
		 * Returns the list of values associated with the switch
		 * represented by this short code, pre-parsed into doubles. If
		 * the switch wasn't set, null is returned.
		 *
		 * @param flag
		 * @return
		 * @throws ArgumentParser.ResultUseException
		...
		/**
		 * Returns the list of values associated with the switch
		 * represented by this long code, pre-parsed into doubles. If
		 * the switch wasn't set, null is returned.
		 *
		 * @param flag
		 * @return
	...
	/**
	 * Adds an argument to this argument parser. This is the most complex
	 * method of adding an argument, all other methods are wrappers around
	 * this.
	 *
	 * The short code and long code for a switch both represent the
	 * underlying switch, that is, they both "addresses" of a single
	 * underlying switch. When accessing the argument later, you may use
	 * either the short code or the long code to retrieve the value of the
	 * switch, but it is important to understand that they are both pointing
	 * to the same item.
	 *
	 ...
	 * @param defaultVal The default value of this switch. If defaultVal is
	 * not null, the switch will always exist when calling get*Argument from
	 * the results. If argType is BOOLEAN, setting this will cause the
	 * switch to
	 * @param description The description of this argument, which is used
	 * when building the help text created by getBuiltDescription.
	 * @return
	 */
	...
	/**
	 * Sets the default switch's arg type, default value, and description.
	 * The default switch is the switch that is associated with "loose" arguments,
	 * for instance, <code>these are args</code> would all be loose arguments,
	 * because they aren't associated with any explicit switches. Note that there
	 * is no Type specified here, that's because the arguments can be grabbed
	 * as either an array of strings or a string.
	 * @param argType
	 * @param defaultVal
	 * @param description
	 * @return 
	 ...
	 * single string. It supports quoted arguments (both single and double),
	 * so "this example" would make the string "this example" one argument
	 * instead of two. Quotes may be escaped with a backslash, like so:
	 * "\"quote\"". Also, all arguments that start with a dash are
	 * considered flags, if you need a literal dash, escape it with a
	 * backslash too, \-. Instead of quoting arguments, you could also add a
	 * \ in front of a space to make it a literal space,
	 * <code>like\ this</code>. Within a string, to add a literal backslash
	 * in front of an otherwise escapable character, use two backslashes
	 * <code>"like this\\"</code>
	 *
	 * @param args
	 * @return
	...
	/**
	 * This method assumes that the arguments have already been parsed out,
	 * so, for instance, if an argument inside of args[x] contains spaces,
	 * it will still be considered one argument. So, ["args with spaces",
	 * "args"] may have looked like this originally:
	 * <code>"args with spaces" args </code> but through some means or
	 * another, you have already parsed the arguments out.
	 *
	 * @param args
	 * @return
	 */
