...
/**
 * This class provides a framework of simple hooks to run critical code on the
 * main thread for code that is running off the main thread.
 *
 * The main operations for code that uses this class are invokeNow and
 * invokeLater, and the main operations for code that implements this class are
 * the abstract methods, and the configuration methods.
 *
 *
 */
public abstract class ThreadPump {
     ...
     *
     * @param minHoldTime The minimum amount of time to wait for the next
     * operation before giving up control of the main thread again. This should
     * be set to 0 if there is no or negligible penalty for regaining the main
     * thread once given up.
     * @param maxHoldTime The max time that the main thread should be
     * monopolized before it is given up. This is useful to prevent starvation.
     * @param waitTime The amount of time to wait before resuming if the
     * maxHoldTime was met, and thread control was returned.
     */
    ...
    /**
     * Starts a transaction. This resets all the mechanisms for wait times. For
     * each start() call, there must be exactly one stop() call, however, they
     * may be called multiple times, and only when the last stop() is called
    ...
    /**
     * Stops a transaction, and immediately returns control to the main thread,
     * if this is the last stop() method to be called.
     */
    ...
    /**
     * Stops a transaction, but waits for all tasks to complete before
     * returning. This should ONLY be called if this is the top level stop, and
     * an exception will be thrown if the start stack is not 1.
     */
    ...
    /**
     * Runs a task on the main thread at some point. This returns immediately.
     * Tasks queued up will be run in order, but at some indeterminate time in
     * the future.
     *
     * @param runnable
     */
     ...
     * submitted are queued up in order, so the task may not get run
     * immediately, and the task completion time is still dependant on the
     * thread starvation parameters.
     *
     * @param callable
     * @return
     */
