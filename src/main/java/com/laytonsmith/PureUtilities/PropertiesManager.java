...
/**
 * This class is meant to be a nearly drop in replacement to the {@link java.util.Properties} class, but it extends
 * {@code LinkedHashMap<String, String>} instead of {@code Hashtable<Object, Object>}. It is intended for string based
 * properties files, as opposed to object based properties.
 */
public class PropertiesManager extends LinkedHashMap<String, String> {
	 ...
	 * <i>natural lines</i> and <i>logical lines</i>. A natural line is defined as a line of characters that is
	 * terminated either by a set of line terminator characters ( <code>\n</code> or <code>\r</code> or
	 * <code>\r\n</code>) or by the end of the stream. A natural line may be either a blank line, a comment line, or
	 * hold all or some of a key-element pair. A logical line holds all the data of a key-element pair, which may be
	 * spread out across several adjacent natural lines by escaping the line terminator sequence with a backslash
	 * character <code>\</code>. Note that a comment line cannot be extended in this manner; every natural line that is
	 * a comment must have its own comment indicator, as described below. Lines are read from input until the end of the
	 * stream is reached.
	 *
	 * <p>
	 ...
	 * ignored and do not encode key-element information. In addition to line terminators, this format considers the
	 * characters space ( <code>' '</code>, <code>'&#92;u0020'</code>), tab ( <code>'\t'</code>,
	 * <code>'&#92;u0009'</code>), and form feed ( <code>'\f'</code>, <code>'&#92;u000C'</code>) to be white space.
	 *
	 * <p>
	 ...
	 * but not including, the first unescaped <code>'='</code>, <code>':'</code>, or white space character other than a
	 * line terminator. All of these key termination characters may be included in the key by escaping them with a
	 ...
	 * element string; if there are no remaining characters, the element is the empty string <code>&quot;&quot;</code>.
	 * Once the raw character sequences constituting the key and element are identified, escape processing is performed
	 * as described above.
	 *
	 * <p>
	 ...
	 * error; the backslash is silently dropped. For example, in a Java string the sequence <code>"\z"</code> would
	 * cause a compile time error. In contrast, this method silently drops the backslash. Therefore, this method treats
	 * the two character sequence <code>"\b"</code> as equivalent to the single character <code>'b'</code>.
	 *
