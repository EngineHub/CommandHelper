...
//		displayData(data, dataSources);
	...
	/**
	 * This version is the first thing sent across the wire. If the versions don't match, the connection won't succeed.
	 */
	...
	/**
	 * Once the data is loaded, however that may take place, the data should be sent here, which will load the data into
	 * the viewer.
	 *
	 * @param data
	 */
	...
	/**
	 * Sets the status label. If the current operation is happening because the system is actively doing something,
	 * thinking should be set to true, and the progress bar will be set to visible (but will keep its existing state)
	 * otherwise, the progress bar is hidden.
	 *
	 * @param statusString
	 * @param thinking
	 */
