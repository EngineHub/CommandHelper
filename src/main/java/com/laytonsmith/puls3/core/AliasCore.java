...
import com.laytonsmith.puls3.core.events.EventUtils;
import com.laytonsmith.puls3.core.functions.IncludeCache;
...
public class AliasCore {
    ...
    public static Puls3Plugin parent;
     ...
     */
    public AliasCore(File aliasConfig, File prefFile, File mainFile, PermissionsResolverManager perms, Puls3Plugin parent) throws ConfigCompileException {
    ...
    public boolean alias(String command, final MCCommandSender player, ArrayList<Script> playerCommands) {
        ...
        try { //catch RuntimeException
                                        ...
                                        if (output != null) {
                                            if (!output.trim().equals("") && output.trim().startsWith("/")) {
                                                if ((Boolean) Static.getPreferences().getPreference("debug-mode")) {
                                                    if (player instanceof MCPlayer) {
                                                        Static.getLogger().log(Level.INFO, "[Puls3]: Executing command on " + ((MCPlayer) player).getName() + ": " + output.trim());
                                                    } else {
                                                        Static.getLogger().log(Level.INFO, "[Puls3]: Executing command from console equivalent: " + output.trim());
                                                    }
                                                }
            ...
            throw new InternalException("An error occured in the Puls3 plugin: " + e.getMessage() + Arrays.asList(e.getStackTrace()));
        }
        return match;
    }
        ...
        try {
            ...
            if (!aliasConfig.exists()) {
                ...
                try {
                    ...
                    logger.log(Level.WARNING, "Puls3: Could not write sample config file");
                }
            }
            ...
            if(!mainFile.exists()){
                ...
                try{
                    ...
                    logger.log(Level.WARNING, "Puls3: Could not write sample main file");
                }
            }
            ...
            try{
                ...
                MScriptCompiler.execute(MScriptCompiler.compile(MScriptCompiler.lex(main, mainFile)), main_env, new MScriptComplete() {
                    ...
                    public void done(String output) {
                        logger.log(Level.INFO, "[Puls3]: Main file processed");
                    }
                }, null);
            } catch(ConfigCompileException e){
                logger.log(Level.SEVERE, "[Puls3]: Main file could not be compiled, due to a compile error: " + e.getMessage());
            } catch(ConfigRuntimeException e){
                logger.log(Level.SEVERE, "[Puls3]: Main file did not finish running, due to a runtime error: " + e.getMessage() + ":" + e.getExceptionType() + ":" + e.getLineNum());
            }
            ...
            for (Script s : scripts) {
                try {
                    ...
                    logger.log(Level.SEVERE, "[Puls3]: " + e.toString() + "\nCompilation will continue.");
                }
            }
            int errors = 0;
            for (Script s : scripts) {
            ...
            if (errors > 0) {
                System.out.println("[Puls3]: " + (scripts.size() - errors) + " alias(es) defined, with " + errors + " aliases with compile errors.");
            } else {
                System.out.println("[Puls3]: " + scripts.size() + " alias(es) defined.");
            }
            is_loaded = true;
        } catch (ConfigCompileException ex) {
            logger.log(Level.SEVERE, "[Puls3]: " + ex.toString());
        } catch (IOException ex) {
            logger.log(Level.SEVERE, "[Puls3]: Path to config file is not correct/accessable. Please"
