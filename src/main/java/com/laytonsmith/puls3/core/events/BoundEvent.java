...
//        root.setRoot(tree);
    ...
    public class ActiveEvent{
        ...
        private Priority consumedAt = null;
        private Priority lockedAt = null;
        private List<GenericTreeNode<Construct>> whenCancelled = new ArrayList<GenericTreeNode<Construct>>();
        private List<GenericTreeNode<Construct>> whenTriggered = new ArrayList<GenericTreeNode<Construct>>();
        ...
        public void consume(Priority p){
            consumedAt = p;
        }
        ...
        public Priority lockedAt(){
            return lockedAt;
        }
        
        public void addWhenTriggered(GenericTreeNode<Construct> tree){
            whenTriggered.add(tree);
        }
        
        public void addWhenCancelled(GenericTreeNode<Construct> tree){
            whenCancelled.add(tree);
        }
        
        public void executeTriggered(Env env){
            for(GenericTreeNode<Construct> tree : whenTriggered){
                try {
                    MScriptCompiler.execute(tree, env.clone(), null, null);
        ...
        public void executeCancelled(Env env){
            for(GenericTreeNode<Construct> tree : whenCancelled){
                try {
                    MScriptCompiler.execute(tree, env.clone(), null, null);
                } catch (CloneNotSupportedException ex) {
                    Logger.getLogger(BoundEvent.class.getName()).log(Level.SEVERE, null, ex);
