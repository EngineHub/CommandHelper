...
public class Meta {
    ...
    public static class runas implements Function {
            ...
            if (args[0].val().equals("~op")) {
                ...
                if ((Boolean) Static.getPreferences().getPreference("debug-mode")) {
                    if (env.GetCommandSender() instanceof MCPlayer) {
                        Static.getLogger().log(Level.INFO, "[Puls3]: Executing command on " + env.GetPlayer().getName() + ": " + args[1].val().trim());
                    } else {
                        Static.getLogger().log(Level.INFO, "[Puls3]: Executing command from console equivalent: " + args[1].val().trim());
                    }
                }
                ...
                if (m != null && m.isOnline()) {
                    if (env.GetCommandSender() instanceof MCPlayer) {
                        Static.getLogger().log(Level.INFO, "[Puls3]: Executing command on " + env.GetPlayer().getName() + ": " + args[0].val().trim());
                    } else {
                        Static.getLogger().log(Level.INFO, "[Puls3]: Executing command from console equivalent: " + args[0].val().trim());
                    }
                    //m.chat(cmd);
        ...
        protected void setOp(MCCommandSender player, Boolean value) {
            ...
            try {
                ...
                Static.getLogger().log(Level.WARNING, "[Puls3]: Failed to OP player " + player.getName());
            }
        }
    ...
    public static class call_alias implements Function {
        ...
        public String docs() {
            return "void {cmd} Allows a Puls3 alias to be called from within another alias. Typically this is not possible, as"
