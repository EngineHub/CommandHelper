...
public class BukkitWeatherListener implements Listener {
	...
	public void onLightningStrike(LightningStrikeEvent event) {
		EventUtils.TriggerListener(Driver.LIGHTNING_STRIKE, "lightning_strike",
				new BukkitWeatherEvents.BukkitMCLightningStrikeEvent(event), CommandHelperPlugin.getCore().getLastLoadedEnv());
	}

	@EventHandler(priority = EventPriority.LOWEST)
	public void onThunderChange(ThunderChangeEvent event) {
		EventUtils.TriggerListener(Driver.THUNDER_CHANGE, "thunder_change",
				new BukkitWeatherEvents.BukkitMCThunderChangeEvent(event), CommandHelperPlugin.getCore().getLastLoadedEnv());
	}

	@EventHandler(priority = EventPriority.LOWEST)
	public void onWeatherChange(WeatherChangeEvent event) {
		EventUtils.TriggerListener(Driver.WEATHER_CHANGE, "weather_change",
				new BukkitWeatherEvents.BukkitMCWeatherChangeEvent(event), CommandHelperPlugin.getCore().getLastLoadedEnv());
	}
}
