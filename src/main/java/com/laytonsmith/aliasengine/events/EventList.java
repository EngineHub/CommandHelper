...
public class EventList {
    private static final Map<Type, SortedSet<Event>> event_list =
            new EnumMap<Type, SortedSet<Event>>(Type.class);
    static {
     ...
     * @return 
     */
    public static SortedSet<Event> GetEvents(Type type){
    ...
    public static Event getEvent(String name){
        for(Type type : event_list.keySet()){
            SortedSet<Event> set = event_list.get(type);
            Iterator<Event> i = set.iterator();
            while(i.hasNext()){
                Event e = i.next();
    ...
    private static void initEvents() {
        ...
        for(Class c : classes){
            String apiClass = (c.getEnclosingClass() != null
                    ...
                    : "<global>");
            if (AbstractEventHandler.class.isAssignableFrom(c)) {
                try {
        ...
        if(a.type() == Implementation.GetServerType()){
            ...
            if(AbstractEventHandler.class.isAssignableFrom(c)){
                 if(Event.class.isAssignableFrom(load)){
                    AbstractEventHandler handler = (AbstractEventHandler) c.newInstance();
                    Constructor loaderConstructor = load.getConstructor(AbstractEventHandler.class);
                    ...
                    AbstractEventMixin mixin = (AbstractEventMixin) mixinConstructor.newInstance(e);
