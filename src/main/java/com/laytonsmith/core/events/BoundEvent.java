...
/**
 * This class represents an actually bound event. When the script runs bind(), a new BoundEvent is created as a closure.
 *
 */
public class BoundEvent implements Comparable<BoundEvent> {
	...
	/**
	 * This is the environment that was set at bind time, not the environment set during run time. The environment is
	 * cloned, so changes to the environment will not affect other code.
	 *
	 * @return
	 */
	...
	/**
	 * Compares two event's IDs, and if they are the same, they should be the actual same event. Since only one event of
	 * a given ID exists, technically == should work on these events.
	 *
	 * @param obj
	 * @return
	 */
...
//	}
	...
	/**
	 * The bound event is essentially an ActiveEvent generator. Because bound events don't change from run to run, it
	 * doesn't make sense to store triggered event specific information with the bound event itself. Instead, when the
	 * event is triggered, an ActiveEvent is generated, stored in the environment, and then the script is triggered.
	 * This ActiveEvent contains both the underlying event (if needed for things like cancellation or other event
	 * manipulation) and the BoundEvent object itself (which can be used to get the event id and other information as
	 * needed). For convenience, the parsed event information is also cached here.
	 */
