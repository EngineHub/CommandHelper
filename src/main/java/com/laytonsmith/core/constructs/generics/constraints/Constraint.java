 ...
 * and the second is a ConstructorConstraint. A third example is a LowerBoundConstraint, which is for constraints
 * defined with <code>T super number</code>, and finally, an exact constraint, which is simply a ClassType.
 * <p>
 * In general, two different Constraints may be contradictory, and when used in combination, erroneous. The solver for
 * this isn't built in to the Constraint class however, since you need to know all constraints in the generic to
 * determine if the combination is erroneous. Individually, a constraint cannot itself be erroneous.
 * <p>
 ...
 * as a wildcard.
 * <p>
 * Note that while the class implements Comparable, the order is not well defined, and is subject to change. This is an
 * implementation detail to allow lists of Contraint objects to be ordered in a normal, deterministic way, in order to
 * be properly comparable against other lists of Constraints. The specific order should not be relied upon beyond the
...
public abstract class Constraint implements Comparable<Constraint> {
	...
	/**
	 * @return The name of the type, for instance T. If defined as a wildcard, this will be <code>?</code>.
	 */
	...
	/**
	 * @return A Set which contains the locations where this is valid to be defined at.
	 */
	...
	/**
	 * @return True if the type was defined as a wildcard.
	 */
	...
	/**
	 * @return The code target where this constraint was defined.
	 */
	...
	/**
	 * Returns true if the provided constraint is within the bounds defined by this constraint, false if it isn't. Given
	 * that class Z extends class Y which extends class X, and class Z has a public no arg constructor, consider the
	 * following class definition: <code>class C&lt;T extends X & new T()&gt;</code> and the LHS definition:
	 * <code>C&lt;? extends Y & new ?()&gt;</code> and the RHS <code>new C&lt;Z&gt;()</code>.
	 *
	 * This code is perfectly valid, however, when validating the LHS, we will compare the LHS constraint
	 * <code>new ?()</code> to the definition constraint <code>? extends Y</code>. These don't compare at all, because
	 * we don't have enough information to compare these, however, comparing the RHS <code>Z</code> to both
	 * <code>? extends Y</code> and <code>new ?()</code>, they both do apply.
	 * <p>
	...
	/**
	 * Returns true if type unions can be used in the LHS of this value. Note that in no case can type unions be used on
	 * the RHS, since those require a concrete type.
	 *
	...
	/**
	 * Given the Diamond Operator on the RHS, takes this LHS object and converts it to an ExactType object, which can be
	 * used on the RHS. Note that this only works with one constraint, multiple constraints cannot be inferred, by
	 * definition.
	 *
	 * @param t The code target
