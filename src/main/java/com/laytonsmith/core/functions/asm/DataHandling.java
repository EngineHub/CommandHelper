...
//			int id = llvmenv.getNewLocalVariableReference();
//
...
//			if(nodes.length == 3) {
//				offset = 1;
//				if(!(nodes[offset].getNode().getData() instanceof IVariable)) {
//					throw new CRECastException(getName() + " with 3 arguments only accepts an ivariable as the second argument.", t);
//				}
//				name = ((IVariable) nodes[offset].getNode().getData()).getVariableName();
...
//					}
//				}
//				type = ArgumentValidation.getClassType(nodes[0].getNode().getData(), t);
//			} else {
//				offset = 0;
//				if(!(nodes[offset].getNode().getData() instanceof IVariable)) {
