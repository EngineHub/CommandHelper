...
/**
 * A script is a section of code that has been preprocessed and split into
 * separate commands/actions. For instance, the config script:
 *
 * /command = /cmd
 *
 * /test = /test
 *
 * would be two seperate scripts, the first being the /command, and the second
 * being /test. Certain key information is stored in the Script class. First,
 * the information needed to see if a target string should trigger this script.
...
//                }
			...
			try {
				...
				for (int i = 0; i < a.length; i++) {
					...
					if (env.getEnv(GlobalEnv.class).GetCustom("array_get_alt_mode") == Boolean.TRUE && i == 0) {
						continue;
					}
		...
		for (int j = 0; j < scripts.size(); j++) {
			...
			for (int k = 0; k < thisCommand.size(); k++) {
				try {
					...
					/**
					 * The two commands: /cmd $var1 [$var2] /cmd $var1 would
					 * cause this exception to be thrown, but the signatures are
					 * the same, so the fact that they've matched this far means
					 * they are ambiguous. However, /cmd $var1 $var2 /cmd $var1
					 * is not ambiguous
					 */
