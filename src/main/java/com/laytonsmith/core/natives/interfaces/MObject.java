...
/**
 * An MObject is a definition of an array with a certain configuration. While an MObject can be constructed directly,
 * this is generally discouraged, since the reusability is lower this way. Instead, subclass it, add direct accessors
 * for various properties, and construct that instead. The static methods provided allow for creation of an instance,
 * given a CArray.
 *
 ...
 *
 * If an object contains fields that should not be handled as instance fields, they must be set to private, and they
 * will be ignored. All other fields should be public. If a field MUST be public, but not accessible, you may annotate
 * it with the {@link nofield} annotation, though this is highly discouraged, since it breaks future compatibility.
 *
 * Note that in the example, an Integer is used. This is appropriate, though Construct types may be used as well. The
 * supported POJO types are only the following, and conversions are automatic: All 8 primitives' object wrappers,
 * String, MList, MObject (and subclasses) and MMap.
 *
 */
public class MObject {
	...
	/**
	 * Constructs a fully dynamic MObject based on the given array. This is discouraged from direct use.
	 *
	 * @param data
	 * @return
	 */
	 ...
	 * return null, and the parameter will be added as a dynamic parameter, or if it actually represents a field, that
	 * is set instead. If both the value and the alias are set, the actual value takes priority, should the two values
	 * be different.
	 *
	 * @param field
	 * @return
