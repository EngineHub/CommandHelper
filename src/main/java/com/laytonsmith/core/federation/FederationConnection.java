...
/**
 * A FederationConnection represents the client side of a FederationConnection. All reads and writes should go through
 * this class. The underlying socket is managed via this class. If the socket isn't connected,
 */
public final class FederationConnection {
	...
	/**
	 * Write a line to the connection, but first, ensures the connection is valid. \n is considered to be the line
	 * ending character, not \r or \n\r.
	 *
	 * @param line
	...
	/**
	 * Reads a line from the connection, but first, ensures the connection is valid. \n is considered to be the line
	 * ending character, not \r or \n\r.
	 *
	...
	/**
	 * Writes raw bytes to the connection, but first, ensures the connection is valid.
	 *
	 * @param bytes
	 * @throws java.io.IOException
	 */
	...
	/**
	 * Reads raw bytes from the connection, but first, ensures the connection is valid.
	 *
	...
	/**
	 * Forces the connection to be established, if it isn't already. This method blocks until it is established (or
	 * fails, and it throws an exception).
	 *
	...
	/**
	 * The socket lock. This should be synchronized on while any socket communication session is occurring, to ensure
	 * that the socket will only be used by one thread at a time.
	 *
	 * @return
	 */
	...
	/**
	 * Closes the socket if it has been inactive for {@code ms} milliseconds. If the socket is closed in this way, true
	 * is returned. Otherwise, if the socket will remain open, false is returned.
	 *
	 * @param ms
	 * @return
	 */
