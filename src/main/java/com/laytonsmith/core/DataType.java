...
/**
 * All constructs have or can return a certain data type. Sometimes it is useful to compare these types at compile time,
 * but since CH isn't strictly type safe, that isn't always practical. However, in many cases, if we simply knew the
 * type that could be returned, if it were invalid, we could cause a compile error at that time. All constructs and
 * functions return a data type, though for many things, it may be mixed. If it is mixed, the type checking must be
 * deferred to runtime, however, in cases where a type is requested, and the type returned is not that, nor can it be
